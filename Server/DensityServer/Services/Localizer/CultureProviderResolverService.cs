...
namespace DensityServer.Services.Localizer
{
    public class CultureProviderResolverService : RequestCultureProvider
    {
        ...
        private static string ParseCookieValue(string cookie)
        {
            if (string.IsNullOrWhiteSpace(cookie))
            {
                return null;
            }
            var parts = cookie.Split(_cookieSeparator, StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length != 2)
            {
                return null;
            }
            ...
            { return null; }
            if (cultureName != null && uiCultureName == null)
            { return uiCultureName = cultureName; }
            if (cultureName == null && uiCultureName != null)
